...
**How we cast it:** a JetStream stream, `WORK`, capturing `work.>`, with each interested
service reading it under its own consumer name. Work assignments are routed to
`work.assigned` and are held.

Two things have to be true together, and either one alone gives you nothing: the subject
has to be captured by a stream, and the listener has to be a named consumer of it.

## There is no durable message storage in this solution

**The role:** hold an envelope that could not be processed, so somebody can look at it
later.

**How we cast it:** we do not. Wolverine's error queue, its inbox and its outbox are all
features of a **durable message store** — a database Wolverine owns — and this solution
has none.

Worth knowing because the logs do not know that. A handler that throws produces
*"was moved to the error queue"*, which names a destination that does not exist here.
Nothing is holding that envelope and nothing will redeliver it.
